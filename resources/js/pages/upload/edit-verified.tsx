...
export default function EditVerifiedUpload({
    uploadType,
    upload,
}: {
    ...
    function saveAllFiles() {
        ...
        setError('');
        router.put(
            `/upload/${uploadType.slug}/uploads/${upload.id}`,
                ...
                onError: (errors) => {
                    ...
                    setError(
                        Object.values(errors)[0] ??
                            'The corrected data could not be saved. Check every field and try again.',
                    );
                },
    ...
    if (!selected || !draft) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 text-slate-900">
                No extracted files are available to edit.
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-950 lg:h-screen lg:overflow-hidden">
            <Head title={`Edit Corrected Data - ${upload.serial_prefix}-${upload.serial_number}`} />
                ...
                <div className="mx-auto flex h-full max-w-[1920px] flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between lg:px-6 lg:py-0">
                    <div className="flex items-center gap-3">
                        <Button asChild variant="ghost" size="sm" className="h-9 px-2.5">
                            <Link href={`/upload/${uploadType.slug}/uploads`}>
                                <ChevronLeft className="mr-1 size-4" /> My uploads
                            </Link>
                        </Button>
                        ...
                        <div>
                            <p className="font-semibold text-sm">Edit Corrected Data</p>
                            <p className="text-slate-500 text-xs">
                                Update verified document extraction fields and line items
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        <div className="shrink-0 border-b p-4">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    <h2 className="font-semibold text-lg">Corrected data</h2>
                                    <p className="text-slate-500 text-sm">
                                        Update any labels or values that were verified incorrectly.
                                    </p>
                                </div>
            ...
            <Dialog open={confirming} onOpenChange={setConfirming}>
                <DialogContent hideCloseButton>
                    <DialogHeader>
                        <DialogTitle>Confirm updated corrected data</DialogTitle>
                        <DialogDescription>
                            Save updated corrections for all {upload.files.length} scanned{' '}
                    ...
                    <div className="flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-950">
                        ...
                        <p className="text-sm">
                            Saving updates the stored verified data for downstream reports and
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={saveAllFiles} disabled={saving}>
        ...
        <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-slate-500 text-xs">
                        Edit labels or values that need correction.
                    </p>
                </div>
                <Button
                    type="button"

...
use App\Http\Requests\Admin\VerifyAdminAccessOtpRequest;
use App\Models\User;
...
class AdminAccessOtpController extends Controller
{
    ...
    public function show(
        Request $request,
        AdminAccessOtpGrant $grant,
        AdminAccessOtpService $otp,
        ...
        if ($remember->check($request, 'admin', $user)) {
            $grant->grant($request);
    ...
    public function verify(
        VerifyAdminAccessOtpRequest $request,
        AdminAccessOtpService $otp,
        AdminAccessOtpGrant $grant,
        OtpDeviceRemember $remember,
    ): RedirectResponse {
        ...
        if ($request->boolean('remember')) {
            $response->withCookie($remember->cookie('admin', $user));
        }

        return $response;
    }

    public function resend(Request $request, AdminAccessOtpService $otp): RedirectResponse
    {
        $user = $request->user();
        abort_unless($user instanceof User, 403);
        try {
        ...
        return back()->with('status', 'A new admin verification code was sent.');
    }

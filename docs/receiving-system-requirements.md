...
### REQ-024 - Google Sheet PO sync and upload-confirmed receiving stock posting

- **Crystallized**: Synchronize Purchase Orders from Google Sheets into durable `po_extractions` and `po_extraction_items` (`source_type = 'google_sheet'`) without requiring PO PDF uploads. A single shared `PurchaseOrderResolver` matches receiving uploads across both uploaded PDF POs and Google Sheet POs, evaluating vendor normalization, reference numbers, date proximity, and line item similarity while preventing ambiguous or conflicting auto-links. Verified live receiving uploads linked to a confirmed PO automatically post dispatchable stock lots (`WarehouseStockLot`) with placement timestamps derived from verified upload completion (`upload_completed_at ?? created_at`), eliminating secondary manual warehouse operator clicks while preserving FIFO dwell attribution and lot ledger invariants. Google Sheet receiving webhooks and sync respect source modes (`parallel`, `reconciliation`, `app_only`), preserve human-verified corrections (`review_status = 'verified'` and `corrected_json`), and enforce timing-safe shared secret authentication.
- **Type**: Functional, Integration, Data Integrity, Security.
- **Architecturally significant**: Yes; unifies PO and receiving data sources, eliminates operational dwell lag, and enforces source boundary transitions.
- **Acceptance**: Live receiving uploads link to valid Google Sheet POs and post dispatchable stock automatically; dwell time accurately reflects arrival without operator lag; sheet sync updates raw JSON while preserving verified corrections; incoming sheet receiving records are blocked in `app_only` mode; unauthenticated webhooks are rejected.
- **Evidence**: `PurchaseOrderSheetSyncTest`, `AutomaticReceiptPostingTest`, `PurchaseOrderLinkingTest`, `GoogleSheetWebhookIngestionTest`, `ReceivingSourceTransitionTest`, and ADR-006.
